...
export async function sendTelegramToAdmins(token, chatIds, text, extra = {}) {
  const results = await Promise.allSettled(chatIds.map((chatId) => sendTelegramMessage(token, chatId, text, extra)));
...
export async function generateOpenRouterText({ apiKey, model, mode, prompt, history = [] }) {
  const response = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({
      model,
      messages: [{ role: "system", content: getModePrompt(mode) }, ...history, { role: "user", content: prompt }],
