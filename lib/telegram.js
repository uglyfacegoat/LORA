...
export function buildLeadActions({ email, dialCode, phone }) {
  ...
  const safePhone = `${asText(dialCode)}${asText(phone)}`.replace(/[^\d+]/g, "");

  if (safeEmail) {
    buttons.push({ text: "Email", url: `mailto:${safeEmail}` });
  }
  if (safePhone) {
    buttons.push({ text: "Call", url: `tel:${safePhone}` });
  }
...
export function getBotMenuText() {
  return [
    "<b>LORA control bot</b>",
    "",
    "Choose a mode:",
    "• <b>AI Chat</b> — general assistant for the team",
    "• <b>Client Reply</b> — generate a polished response to a lead",
    "• <b>Follow-up</b> — generate a follow-up message",
    "• <b>Offer Outline</b> — outline a proposal / brief structure",
    "",
...
export function getBotMenuMarkup() {
  return {
    inline_keyboard: [
      [
        { text: "AI Chat", callback_data: "mode:chat" },
        { text: "Client Reply", callback_data: "mode:reply" },
      ],
      [
        { text: "Follow-up", callback_data: "mode:followup" },
        { text: "Offer Outline", callback_data: "mode:offer" },
      ],
      [
        { text: "Reset", callback_data: "mode:reset" },
        { text: "Help", callback_data: "mode:help" },
      ],
    ],
  };
}
